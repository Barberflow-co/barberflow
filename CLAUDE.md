...
- **Tenant identification: ALWAYS prefer `auth_user_id` over `email` in tenant lookups.** Email is for human-readable display only. The `equipe` table has both columns — `auth_user_id` is the strong unique link to the Supabase Auth account. Falling back to email is acceptable only for one-time auto-link of a *single* pending invite (`auth_user_id IS NULL`); if there are multiple pending records for the same email, block and surface a manual-resolution error. See `migrations/001_tenant_isolation_audit.sql` for the audit script.
